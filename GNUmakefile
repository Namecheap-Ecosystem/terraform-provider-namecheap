version = 99.0.0
provider_macos_path = registry.terraform.io/Namecheap-Ecosystem/miro/$(version)/darwin_amd64/
...
install_macos:
	@go build -o terraform-provider-namecheap_$(version)
	@mkdir -p ~/Library/Application\ Support/io.terraform/plugins/$(provider_macos_path)
	@mv terraform-provider-namecheap_$(version)  ~/Library/Application\ Support/io.terraform/plugins/$(provider_macos_path)
